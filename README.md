...
> **Status: paused (last active Jul 2026) — exploratory prototype.** Stopped because current multimodal LLMs cannot read construction drawings and site photos at the precision quality inspection requires; to be revisited when model capability catches up. Personal project, sole developer. Code and data are not published (the drawings and inspection records belong to a completed project I worked on).
...
The seven-step chain is: ① parse location → ② grid coordinates → ③ select the right drawing → ④ retrieve the region → **⑤ read the drawing / photo** → ⑥ cross-check → ⑦ report. Steps ①–④ work. Step ⑤ is where the prototype failed, on all three fronts tested:
...
The honest assessment at the pause point: without step ⑤, the system is an advanced document-retrieval and field-lookup tool for one project, not an inspector. That is useful, but not the product.
...
Choose the killer feature by what an experienced engineer actually does (find, not fix); build the deterministic parts first so the model's contribution can be measured in isolation; and stop when the measurement says the missing capability is the model's, not the pipeline's.
